...
ssh -o UserKnownHostsFile=/config/.ssh/known_hosts -i /config/.ssh/id_rsa -f root@192.168.34.130 'rm /config/sh/result.log; nohup /config/sh/remote/activeZigbee2mqtt1.sh >/config/sh/result.log 2>&1 </dev/null &'
...
#cd /config/sh/
#rm ./nohup.out
#nohup ./activeZigbee2mqtt1_background.sh & disown
