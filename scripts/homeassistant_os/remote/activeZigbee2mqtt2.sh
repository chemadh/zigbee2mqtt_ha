...
if echo "$command" | bash ; then
        ...
        command="ssh -o UserKnownHostsFile=/config/.ssh/known_hosts -i /config/.ssh/id_rsa ${zigbee2mqtt1_remote_ssh} 'rsync -avuP ${zigbee2mqtt_nvram_path}${zigbee2mqtt_nvram_file} ${zigbee2mqtt2_remote_ssh}:${zigbee2mqtt_nvram_path}'"
        echo "$command"
        if echo "$command" | bash ; then
...
command="ssh -o UserKnownHostsFile=/config/.ssh/known_hosts -i /config/.ssh/id_rsa ${zigbee2mqtt2_remote_ssh} 'sudo rsync -rlpgoDcv ${local_aux_dir}* ${local_conf_dir}'"
echo "$command"
if echo "$command" | bash ; then
...
command="ssh -o UserKnownHostsFile=/config/.ssh/known_hosts -i /config/.ssh/id_rsa ${zigbee2mqtt2_remote_ssh} '${zigbee2mqtt_start_cmd}'"
echo "$command"
if echo "$command" | bash ; then
...
command="ssh -o UserKnownHostsFile=/config/.ssh/known_hosts -i /config/.ssh/id_rsa ${zigbee2mqtt1_remote_ssh} 'sudo reboot'"
echo "$command"
if echo "$command" | bash ; then
